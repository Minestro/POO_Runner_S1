...
Player::Player(unsigned int i, std::string name): m_bestScore{0}, m_nickName{name}, m_money{0}, m_id{i}, m_powerUpgrades{}
{
    for (unsigned int i = 0; i<power_list::NB_POWER; i++)
    {
        m_powerUpgrades[i] = (DEFAULT_UPGRADE_SEC * 1000);
...
void Player::saveProfile() const
{
    ...
    for (unsigned int i=0; i<power_list::NB_POWER; i++)
    {
        if (el->FirstChildElement(std::string("Upgrade" + std::to_string(i)).c_str()) == nullptr)
        {
            throw XMLError (XML_ERROR_PARSING_ELEMENT);
        }
        el->FirstChildElement(std::string("Upgrade" + std::to_string(i)).c_str())->SetText(m_powerUpgrades[i]);
    }
...
void Player::eraseProfil(unsigned int i)
{
    ...
    for (unsigned int i=0; i<power_list::NB_POWER; i++)
    {
        ...
        el->FirstChildElement(std::string("Upgrade" + std::to_string(i)).c_str())->SetText("");
    }
    if (el->FirstChildElement("BestScore") == nullptr)
    {
        throw XMLError (XML_ERROR_PARSING_ELEMENT);
